...
##How to fork the repo and make a pull request, contribute
...
##Brief description of the folder structure each page has their own folder in components
##Setting up prettier in vscode and auto format on save
Ways to Install Prettier as a VS Code Extension:

Please paste the below given Commands in Terminal:

1. Using npm:
   ```npm install --save-dev --save-exact prettier```
2. Enable Prettier on VS Code:
   ```ext install esbenp.prettier-vscode```
3. Or directly go to:
   ```VS Code→ Extensions→Search for Prettier - Code formatter```
